...
US-092 + US-093 (Both complete)
  ...
  └─ BLOCKS → US-094 (Transition Validation - 3 weeks)
                 │
                 └─ BLOCKS → US-095 (Retire code-searcher)
                                │
                                └─ BLOCKS → US-096 (Archive code-searcher.md)
```
...
**Milestone**: code-searcher responsibilities migrated to architect + code_developer
**Unblocks**:
...
- Rollback plan: Keep code-searcher available during validation
