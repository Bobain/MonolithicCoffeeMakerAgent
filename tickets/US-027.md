...
```
main branch:
  docs/ROADMAP.md (outdated - waiting for PR merge)

feature/us-021:
  docs/ROADMAP.md (modified - but not visible to others)

roadmap branch:
  docs/ROADMAP.md (maybe updated, maybe not - unclear!)
    ...
    steps:
      ...
      - name: Check if ROADMAP.md on roadmap is newer
        run: |
          ...
          roadmap_date=$(git log origin/roadmap -1 --format=%ct -- docs/ROADMAP.md)
          pr_date=$(git log HEAD -1 --format=%ct -- docs/ROADMAP.md)
...
cat docs/ROADMAP.md | grep "📝 Planned"  # Check priorities
...
# Fix conflicts in docs/ROADMAP.md
git add docs/ROADMAP.md
